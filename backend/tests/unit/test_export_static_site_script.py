...
def test_static_rrg_bootstrap_runs_when_history_is_short(monkeypatch, tmp_path):
    calls: dict[str, object] = {}
    fake_db = object()
    short_state = SimpleNamespace(weeks=(object(),))
    ...
    class FakeHistoryService:
        def prepare(self, db, **kwargs):
            calls["prepare"] = {"db": db, **kwargs}
            return SimpleNamespace(
                state=short_state,
                warnings=("existing history is short",),
                unavailable_reason=None,
            )

        def has_minimum_history(self, state):
            calls["readiness_state"] = state
            return False

    class FakeBootstrapService:
        def backfill(self, db, **kwargs):
            calls["backfill"] = {"db": db, **kwargs}
            return SimpleNamespace(
                as_dict=lambda: {
                    "status": "completed",
                    "market": "US",
    ...
    monkeypatch.setattr(export_script, "SessionLocal", fake_session)
    monkeypatch.setattr(export_script, "StaticRRGHistoryBundleService", FakeHistoryService)
    monkeypatch.setattr(export_script, "StaticRRGBootstrapBackfillService", FakeBootstrapService)

    result = export_script._bootstrap_static_rrg_history_if_needed(  # noqa: SLF001
        rrg_history_dir=tmp_path,
        market="US",
        as_of_date=date(2026, 7, 17),
        formula_version=BALANCED_RS_FORMULA_VERSION,
    )
    ...
    assert calls["readiness_state"] is short_state
    ...
    assert result == {
        ...
        "formula_version": BALANCED_RS_FORMULA_VERSION,
        "weeks_before": 1,
        "readiness_warnings": ["existing history is short"],
    }


def test_static_rrg_bootstrap_skips_when_history_is_sufficient(monkeypatch, tmp_path):
    calls: list[str] = []
    fake_db = object()
    ready_state = SimpleNamespace(weeks=tuple(object() for _ in range(12)))
    ...
    class FakeHistoryService:
        ...
        def has_minimum_history(self, state):
            assert state is ready_state
            return True

    class BootstrapShouldNotRun:
        def backfill(self, *_args, **_kwargs):
            calls.append("backfill")
            raise AssertionError("bootstrap should not run with sufficient history")

    monkeypatch.setattr(export_script, "SessionLocal", fake_session)
    monkeypatch.setattr(export_script, "StaticRRGHistoryBundleService", FakeHistoryService)
    monkeypatch.setattr(export_script, "StaticRRGBootstrapBackfillService", BootstrapShouldNotRun)
...
def test_run_daily_refresh_bootstraps_rrg_history_before_market_consumers(
    monkeypatch,
    tmp_path,
):
    calls: list[str] = []
    ...
    monkeypatch.setattr(
        export_script,
        "_ensure_group_rank_history",
        lambda *, as_of_date, market, formula_version: calls.append(
            f"group:{market}:{as_of_date.isoformat()}"
        )
        or _backfill_result(
            status=GroupRankHistoryBackfillStatus.ERRORED,
            market=market,
            as_of_date=as_of_date,
            error="historical lifecycle evidence is missing",
        ),
    )

    def fake_bootstrap(*, rrg_history_dir, market, as_of_date, formula_version):
        calls.append(f"rrg_bootstrap:{market}:{as_of_date.isoformat()}")
        assert rrg_history_dir == tmp_path
        assert formula_version == BALANCED_RS_FORMULA_VERSION
        return {"status": "completed", "market": market}

    monkeypatch.setattr(
        export_script,
        "_bootstrap_static_rrg_history_if_needed",
        fake_bootstrap,
    )
    monkeypatch.setattr(
        export_script,
        "_compute_static_market_exposure",
        lambda *, as_of_date, market: calls.append(
            f"exposure:{market}:{as_of_date.isoformat()}"
        )
        or {"market": market, "date": as_of_date.isoformat(), "exposure_score": 42.0},
    )
    monkeypatch.setattr(
    ...
    assert calls == [
        "price:US:2026-06-25",
        "group:US:2026-06-25",
        "rrg_bootstrap:US:2026-06-25",
        "exposure:US:2026-06-25",
        "snapshot:US:2026-06-25",
    ]
    assert results["rrg_bootstrap_backfill"] == {
        "US": {"status": "completed", "market": "US"}
    }
...
def test_main_passes_rrg_history_dir_to_daily_refresh(monkeypatch, tmp_path):
    captured: dict[str, object] = {}
    rrg_history_dir = tmp_path / "rrg"

    class FakeRollingRRGSession:
        warnings = ()

        def __init__(self, **kwargs):
            captured["rrg_session"] = kwargs

        def persist(self, *, exported_as_of_date):
            captured["persisted_as_of_date"] = exported_as_of_date
            return None

    class FakeExportService:
        def __init__(self, *_args, **kwargs):
            captured["export_service"] = kwargs
    ...
    def fake_refresh(**kwargs):
        ...
        return (
            {
                "feature_snapshots": {
                    "US": {"status": "published", "market": "US", "run_id": 91}
                },
                "market_exposure": {
                    "US": {
                        "market": "US",
    ...
    monkeypatch.setattr(export_script, "prepare_runtime", lambda: None)
    monkeypatch.setattr(export_script, "_run_daily_refresh", fake_refresh)
    monkeypatch.setattr(
        export_script,
        "StaticGroupsRRGRollingHistoryExportSession",
        FakeRollingRRGSession,
    )
    monkeypatch.setattr(export_script, "StaticSiteExportService", FakeExportService)
    ...
    assert result == 0
    assert captured["refresh"]["rrg_history_dir"] == rrg_history_dir
    assert captured["rrg_session"]["directory"] == rrg_history_dir
    assert captured["persisted_as_of_date"] == date(2026, 4, 5)
